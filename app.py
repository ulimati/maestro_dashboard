...
if not st.session_state.show_dashboard and not st.session_state.show_empty:
    st.markdown("""
        <div style="text-align: center; padding: 80px 0 40px 0;">
            ...
            <p style="color: #8e8e93;">Select a project to view test results</p>
        </div>
    """, unsafe_allow_html=True)

    col1, col2, col3, col4, col5 = st.columns([2, 4, 1, 4, 2])
    with col2:
        if st.button("Project Alpha", use_container_width=True):
            st.session_state.show_dashboard = True
            st.session_state.current_project = "Project Alpha"
            st.rerun()
    with col4:
        if st.button("Project Beta", use_container_width=True):
            st.session_state.show_empty = True
            st.session_state.current_project = "Project Beta"
            st.rerun()
    st.stop()

# =================================================================
# PROJECT BETA – prázdná stránka
# =================================================================
if st.session_state.show_empty:
    if st.button("← Back"):
        st.session_state.show_empty = False
        st.rerun()
    st.title("Project Beta")
    st.info("No test data available for this project yet.")
    st.stop()

# =================================================================
# SIDEBAR
# =================================================================
st.sidebar.title("Maestro Dashboard")
st.sidebar.caption(f"Project: {st.session_state.current_project}")
...
if "API" in platform:
    ...
    with tab_all:
        ...
        for i, name in enumerate(unique_tests):
            ...
            with cols[i % 2]:
                with st.expander(f"{folder_icon} 📁 {name} ({len(test_data)} runs)"):
                    for _, row in test_data.iterrows():
                        run_icon = "✅" if row["status"] == "Passed" else "❌"
                        with st.container(border=True):
                            st.write(f"**{run_icon} Run ID:** `{row.get('run_id', 'N/A')}`")
                            st.caption(f"Duration: {row.get('duration', 0)}s")
                        if row["status"] == "Failed":
                            st.error(f"Error: {row.get('error_msg', 'Unknown error')}")
...
with col_cal:
    events = [
        {"title": "", "start": d, "display": "block", "color": "#ff453a"}
    ...
    for i, name in enumerate(unique_tests):
        ...
        with cols[i % 2]:
            with st.expander(f"{icon} {name} ({total_in_test} {label})"):
                ...
                if len(test_data) > 20:
                    st.caption(f"⚠️ Showing 20 of {len(test_data)} records.")
...
with st.expander("📊 Global history (entire platform)", expanded=True):
    ...
    clicked_status_global = render_charts(df, key_suffix="global", show_selector=True)
...
if selected_run and selected_run != "No runs available":
    ...
    with st.expander(f"🔎 Run detail: {selected_run}", expanded=True):
        ...
        if clicked_status_single:
            ...
            run_df_filtered = run_df[run_df["status"] == clicked_status_single].reset_index(drop=True)
            cols = st.columns(2)
